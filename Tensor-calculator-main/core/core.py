...
class compute():
        ...
        for sigma in range(n):
            for mu in range(n):
                for nu in range(n):
                    ...
                    Gamma[sigma][mu][nu] = Simple.custom_simplify(sp.Rational(1, 2) * Gamma_sum)
            ...
            for sigma in range(n):
                for mu in range(n):
                    for nu in range(n):
                        ...
                        Riemann[rho][sigma][mu][nu] = Simple.custom_simplify(term1 - term2 + sum_term)

        R_abcd = index.lower_indices(Riemann, g, n)

        Ricci = sp.zeros(n, n)
        for mu in range(n):
            for nu in range(n):
                Ricci[mu, nu] = Simple.custom_simplify(sum(Riemann[rho][mu][rho][nu] for rho in range(n)))
                Ricci[mu, nu] = Simple.custom_simplify(Ricci[mu, nu])

        Scalar_Curvature = Simple.custom_simplify(sum(g_inv[mu, nu] * Ricci[mu, nu] for mu in range(n) for nu in range(n)))
        Scalar_Curvature = Simple.custom_simplify(Scalar_Curvature)
    ...
    def compute_einstein_tensor(Ricci, Scalar_Curvature, g, g_inv, n):
        ...
        for mu in range(n):
            for nu in range(n):
                G_lower[mu, nu] = Simple.custom_simplify(Ricci[mu, nu] - sp.Rational(1, 2) * g[mu, nu] * Scalar_Curvature)
        ...
        for mu in range(n):
            for nu in range(n):
                ...
                G_upper[mu, nu] = Simple.custom_simplify(sum_term)
